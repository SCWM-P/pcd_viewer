...
def get_overall_xy_bounds(slices_dict):
    all_bounds_xy = []; valid_slice_found = False
    for slice_data in slices_dict.values():
        if slice_data is not None and slice_data.n_points > 0:
            b = slice_data.bounds
            if b[0] < b[1] and b[2] < b[3]:
                 all_bounds_xy.extend(b[0:4]); valid_slice_found = True
    if not valid_slice_found or not all_bounds_xy: return None
    xmin = min(all_bounds_xy[0::4]); xmax = max(all_bounds_xy[1::4])
    ymin = min(all_bounds_xy[2::4]); ymax = max(all_bounds_xy[3::4])
...
def render_slice_to_image(slice_data, size, overall_xy_bounds=None, is_thumbnail=True):
    ...
    try:
        ...
        elif slice_data and slice_data.bounds[0] < slice_data.bounds[1]: bounds_to_use = slice_data.bounds
    ...
    except Exception as e: print(f"ERROR: Error rendering slice: {e}"); return None, {}
    finally:
        if plotter:
            try: plotter.close()
...
def create_density_heatmap(density_matrix, colormap_name='viridis', vmin=None, vmax=None):
    ...
    try:
        ...
        if q_img.isNull():
            print("ERROR: create_density_heatmap - QImage failed."); return QPixmap()
...
class SliceProcessingThread(QThread):
    ...
    def __init__(self, point_cloud, num_slices, thickness, limit_thickness, thumbnail_size, parent=None):
        super().__init__(parent); self.point_cloud = point_cloud; self.num_slices = num_slices
        self.thickness_param = thickness; self.limit_thickness = limit_thickness; self.thumbnail_size = thumbnail_size
    ...
    def run(self):
        ...
        try:
            ...
            for i in range(self.num_slices):
                ...
                else: generated_slices.append(None); self.slice_ready.emit(i, None, (slice_start_z, slice_end_z))
            ...
            for i in range(self.num_slices):
                if not self._is_running: raise InterruptedError("Stopped")
                self.progress.emit(int((self.num_slices + i + 1) / total_steps * 100), f"缩略图 {i+1}/{self.num_slices}")
                ...
                if img_np is not None:
                    try:
                        ...
                        pixmap = QPixmap.fromImage(q_img); scaled_pixmap = pixmap.scaled(self.thumbnail_size, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
                        self.thumbnail_ready.emit(i, scaled_pixmap, metadata)
                    except Exception as qimage_err: print(f"ERROR: QImage/QPixmap failed: {qimage_err}"); placeholder_pixmap = QPixmap(self.thumbnail_size); placeholder_pixmap.fill(Qt.GlobalColor.darkRed); self.thumbnail_ready.emit(i, placeholder_pixmap, metadata)
                else: placeholder_pixmap = QPixmap(self.thumbnail_size); placeholder_pixmap.fill(Qt.GlobalColor.lightGray); painter = QPainter(placeholder_pixmap); painter.drawText(placeholder_pixmap.rect(), Qt.AlignmentFlag.AlignCenter, f"Slice {i}\n(Empty)"); painter.end(); self.thumbnail_ready.emit(i, placeholder_pixmap, metadata)
        ...
        try:
            ...
            for i, index in enumerate(sorted_indices):
                ...
                if slice_data is not None and slice_data.n_points > 0:
                    ...
                    all_matrices[index] = density_matrix; current_max = np.max(density_matrix);
            ...
            for i, index in enumerate(sorted_indices):
                 ...
                 density_params = {"grid_resolution": self.grid_resolution,"colormap": self.colormap_name,"xy_bounds": self.overall_xy_bounds,"max_density_scale": float(max_density)} # Ensure max_density is float for JSON
...
class BatchSliceViewerWindow(QWidget):
    ...
    DIVERGING_COLORMAPS = sorted([cm for cm in AVAILABLE_COLORMAPS if cm.lower() in ['rdbu', 'bwr', 'coolwarm', 'seismic', 'piyg', 'prgn', 'brbg', 'puor']])
    ...
    def setup_left_panel(self):
        ...
        self.slice_list_widget = QListWidget(); self.slice_list_widget.setViewMode(QListWidget.ViewMode.IconMode)
        self.slice_list_widget.setIconSize(QSize(128, 128)); self.slice_list_widget.setResizeMode(QListWidget.ResizeMode.Adjust)
        ...
        select_all_btn = QPushButton("全选"); select_all_btn.clicked.connect(self.slice_list_widget.selectAll)
        deselect_all_btn = QPushButton("全不选"); deselect_all_btn.clicked.connect(self.slice_list_widget.clearSelection)
        export_selected_btn = QPushButton("导出选中"); export_selected_btn.clicked.connect(self._export_selected_data)
        ...
        list_group_layout.addLayout(list_button_layout); left_layout.addWidget(list_group)
    ...
    def setup_center_panel(self):
        ...
        try:
            self.plotter = QtInteractor(parent=self.plotter_widget); plotter_layout.addWidget(self.plotter)
            QTimer.singleShot(200, self._initialize_plotter_view)
        except Exception as e: print(f"ERROR: Failed to create QtInteractor: {e}"); self.plotter = None; error_label = QLabel(f"无法初始化3D视图。\n错误: {e}"); error_label.setAlignment(Qt.AlignmentFlag.AlignCenter); plotter_layout.addWidget(error_label)
    ...
    def setup_view_control_tab(self, layout):
        ...
        self.num_slices_spin.setValue(10); num_slices_layout.addWidget(self.num_slices_spin)
        ...
        self.thickness_spin.setSingleStep(0.01); self.thickness_spin.setValue(0.10)
        self.thickness_spin.setDecimals(3); thickness_layout.addWidget(self.thickness_spin)
        ...
        self.offset_spin.setSingleStep(0.1); self.offset_spin.setValue(0.5)
    ...
    def setup_density_analysis_tab(self, layout):
        ...
        self.density_resolution_combo = QComboBox(); self.density_resolution_combo.addItems(["256x256", "512x512", "1024x1024", "2048x2048"])
        ...
        self.density_colormap_combo = QComboBox(); self.density_colormap_combo.addItems(self.AVAILABLE_COLORMAPS); self.density_colormap_combo.setCurrentText("viridis")
        dcg_layout.addRow("颜色映射 (主):", self.density_colormap_combo)
        # --- NEW: Diff Colormap ---
        self.diff_colormap_combo = QComboBox(); self.diff_colormap_combo.addItems(self.DIVERGING_COLORMAPS); self.diff_colormap_combo.setCurrentText("RdBu")
        dcg_layout.addRow("颜色映射 (差分):", self.diff_colormap_combo)
        # --- End NEW ---
        self.update_density_btn = QPushButton("计算/更新密度图"); self.update_density_btn.clicked.connect(self._start_density_processing)
        dcg_layout.addRow(self.update_density_btn); layout.addWidget(density_calc_group)
        ...
        self.density_display_combo.setEnabled(False); self.density_display_combo.currentIndexChanged.connect(self._update_center_view)
        dg_layout.addRow("显示内容:", self.density_display_combo); layout.addWidget(display_group)

        # Single Logic Operation Group
        logic_op_group = QGroupBox("单次逻辑运算"); log_layout = QVBoxLayout(logic_op_group)
        slice_selection_layout = QHBoxLayout(); fm = QFontMetrics(self.font()) # For text width calculation
        max_label_width = fm.horizontalAdvance("A: 999") + 10 # Calculate width needed for label
        self.slice_a_label = QLabel("A: 未选"); self.slice_a_label.setMinimumWidth(max_label_width)
        clear_a_btn = QPushButton("清除A"); clear_a_btn.clicked.connect(lambda: self._set_logic_operand('A', None))
        slice_selection_layout.addWidget(self.slice_a_label); slice_selection_layout.addWidget(clear_a_btn); log_layout.addLayout(slice_selection_layout)
        slice_b_layout = QHBoxLayout()
        self.slice_b_label = QLabel("B: 未选"); self.slice_b_label.setMinimumWidth(max_label_width)
        clear_b_btn = QPushButton("清除B"); clear_b_btn.clicked.connect(lambda: self._set_logic_operand('B', None))
        slice_b_layout.addWidget(self.slice_b_label); slice_b_layout.addWidget(clear_b_btn); log_layout.addLayout(slice_b_layout)
        op_layout = QHBoxLayout(); op_layout.addWidget(QLabel("操作:"))
        self.logic_op_combo = QComboBox(); self.logic_op_combo.addItems(list(self.LOGIC_OPERATIONS.keys()))
        op_layout.addWidget(self.logic_op_combo); log_layout.addLayout(op_layout)
        self.compute_logic_op_btn = QPushButton("计算单次运算"); self.compute_logic_op_btn.clicked.connect(self._compute_logic_operation); self.compute_logic_op_btn.setEnabled(False)
        log_layout.addWidget(self.compute_logic_op_btn); layout.addWidget(logic_op_group)
        ...
        self.batch_op_combo = QComboBox(); self.batch_op_combo.addItems(list(self.LOGIC_OPERATIONS.keys())) # Share operations
        batch_op_form_layout.addRow("运算类型:", self.batch_op_combo)
        self.batch_op_k_spin = QSpinBox(); self.batch_op_k_spin.setRange(1, 100); self.batch_op_k_spin.setValue(1)
        batch_op_form_layout.addRow("步长 k:", self.batch_op_k_spin)
        self.batch_op_loop_check = QCheckBox("循环应用"); self.batch_op_loop_check.setChecked(False)
        batch_op_form_layout.addRow("", self.batch_op_loop_check)
        bog_layout.addLayout(batch_op_form_layout)
        self.execute_batch_op_btn = QPushButton("执行批量运算"); self.execute_batch_op_btn.clicked.connect(self._start_batch_logic_operation)
        bog_layout.addWidget(self.execute_batch_op_btn)
        # Batch results preview
        self.batch_results_slider = QSlider(Qt.Orientation.Horizontal); self.batch_results_slider.setEnabled(False)
        self.batch_results_slider.valueChanged.connect(self._update_batch_preview_display)
        self.batch_results_label = QLabel("结果: - / -"); self.batch_results_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.play_pause_btn = QPushButton("播放"); self.play_pause_btn.setCheckable(True); self.play_pause_btn.setEnabled(False)
        self.play_pause_btn.toggled.connect(self._toggle_play_batch_results)
        preview_layout = QHBoxLayout(); preview_layout.addWidget(self.batch_results_slider); preview_layout.addWidget(self.play_pause_btn)
        bog_layout.addLayout(preview_layout); bog_layout.addWidget(self.batch_results_label)
    ...
    def _start_slice_processing(self):
        ...
        self.selected_slice_a = None; self.selected_slice_b = None; self._clear_logic_op_results(); self._clear_batch_op_results(); self._update_logic_op_ui(); self._update_batch_op_ui()
        try: self.plotter.clear(); self.plotter.remove_actor("init_text", render=False); self.plotter.add_text("正在生成切片...", position="upper_left", font_size=12, name="status_text"); self.plotter.render(); QApplication.processEvents()
        ...
        self.progress_dialog = QProgressDialog("正在处理切片...", "取消", 0, 100, self); self.progress_dialog.setWindowTitle("切片处理"); self.progress_dialog.setWindowModality(Qt.WindowModality.WindowModal); self.progress_dialog.setAutoClose(True); self.progress_dialog.setAutoReset(True); self.progress_dialog.canceled.connect(self._cancel_processing)
        QTimer.singleShot(50, self.progress_dialog.show)
        self.slice_processing_thread = SliceProcessingThread(self.original_point_cloud, num_slices, thickness, limit_thickness, thumbnail_size)
        self.slice_processing_thread.progress.connect(self._update_progress); self.slice_processing_thread.slice_ready.connect(self._collect_slice_data); self.slice_processing_thread.thumbnail_ready.connect(self._add_thumbnail_item); self.slice_processing_thread.finished.connect(self._slice_processing_finished)
    ...
    def _start_density_processing(self):
        ...
        self.density_matrices.clear(); self.density_pixmaps.clear(); self.density_params.clear(); self.logic_op_result_matrix = None; self.logic_op_result_pixmap = None; self.selected_slice_a = None; self.selected_slice_b = None; self._clear_batch_op_results(); self._update_logic_op_ui(); self._update_batch_op_ui()
        try:
        ...
        except Exception as e: print(f"Warning: Error resetting density combo: {e}")
        self._update_center_view()
        resolution_text = self.density_resolution_combo.currentText();
        try: grid_resolution = int(resolution_text.split('x')[0])
        except: grid_resolution = self.DEFAULT_DENSITY_RESOLUTION
        colormap_name = self.density_colormap_combo.currentText(); overall_xy_bounds = get_overall_xy_bounds(self.current_slices)
    ...
    def _start_batch_logic_operation(self):
        ...
        if num_slices < 2 or k <= 0:
             ...
             self._update_batch_op_ui(); return
        ...
        for i_idx, current_index in enumerate(sorted_indices):
            ...
            if matrix_curr is not None and matrix_next is not None and matrix_curr.shape == matrix_next.shape:
                 ...
                 print(f"DEBUG: Skipping batch op for ({next_index}, {current_index}) due to missing matrix or shape mismatch.")
        ...
        all_result_values = np.concatenate([m.flatten() for m in self.batch_op_results.values()]) if self.batch_op_results else np.array([0])
        ...
        if DEBUG_MODE: print(f"DEBUG: Generating batch result pixmaps. Colormap={result_colormap}, vmin={vmin_res}, vmax={vmax_res}")
        ...
        self.batch_op_params = { "op_str": operation_str, "k": k, "loop": loop, "indices": calculated_indices, "vmin":vmin_res, "vmax":vmax_res, "colormap":result_colormap }
    ...
    def _slice_processing_finished(self, success):
        ...
        if success:
            ...
            self._update_3d_view_presentation(); self.update_density_btn.setEnabled(True) # Enable density calc
        else: (QMessageBox.information if self.progress_dialog and self.progress_dialog.wasCanceled() else QMessageBox.warning)(self,"结果","切片处理取消或失败."); self.update_density_btn.setEnabled(False) # Disable if no slices
        self._update_logic_op_ui(); self._update_batch_op_ui() # Update enable states
    ...
    def _density_processing_finished(self, success):
        ...
        if success:
            ...
            self.density_display_combo.setEnabled(True); first_available_index = next(iter(sorted(self.density_pixmaps.keys())), -1)
            if first_available_index != -1:
                 self.current_density_display_mode = "选中切片"; self.density_display_combo.setCurrentText(self.current_density_display_mode)
                 ...
                 if not self.slice_list_widget.selectedItems() and self.slice_list_widget.count() > 0:
                     self.slice_list_widget.setCurrentRow(first_available_index) # Select the row corresponding to the index if possible
            ...
            else: self.density_view_label.setText("无有效密度图生成")
        else:
            (
    ...
    def _handle_tab_change(self, index):
        ...
        if index == 0: self.center_stacked_widget.setCurrentIndex(0)
        elif index == 1: self.center_stacked_widget.setCurrentIndex(1); self._update_density_display_label()


    def _update_center_view(self):
        # ... (Logic remains the same) ...
        current_tab_index = self.right_tab_widget.currentIndex()
        if current_tab_index == 0: self.center_stacked_widget.setCurrentIndex(0); # if self.plotter: try: self.plotter.render() except: pass
        elif current_tab_index == 1: self.center_stacked_widget.setCurrentIndex(1); self._update_density_display_label()


    def _update_density_display_label(self):
        # ... (Handles "选中切片", "单次运算结果", "批量运算结果") ...
        pixmap_to_show = QPixmap(); display_mode = self.density_display_combo.currentText(); label_text = ""
        if display_mode == "选中切片":
            ...
            elif self.density_pixmaps: target_index = next(iter(sorted(self.density_pixmaps.keys())), -1) # Fallback to first available
            if target_index != -1 and target_index in self.density_pixmaps:
                self.current_density_display_index = target_index
                pixmap_to_show = self.density_pixmaps[target_index]; label_text = f"切片 {target_index} 密度图"
            else: label_text = f"切片密度图不可用 (索引: {target_index})"
        elif display_mode == "单次运算结果":
            if self.logic_op_result_pixmap and not self.logic_op_result_pixmap.isNull(): pixmap_to_show = self.logic_op_result_pixmap; label_text = "单次运算结果"
            else: label_text = "无单次运算结果"
        elif display_mode == "批量运算结果":
            if self.current_batch_op_display_key in self.batch_op_pixmaps: pixmap_to_show = self.batch_op_pixmaps[self.current_batch_op_display_key]; label_text = f"批量结果: {self.current_batch_op_display_key}"
            else: label_text = "无批量运算结果或未选择"
    ...
    def _on_selection_changed(self):
        ...
        if display_mode == "选中切片":
            ...
            if selected_items:
                ...
                elif DEBUG_MODE: print(f"DEBUG: List selected index {first_selected_index}, but no density pixmap found.")
    ...
    def _show_list_context_menu(self, pos: QPoint):
        ...
        action_a.setEnabled(can_select); action_b.setEnabled(can_select)
        if not can_select: action_a.setText("设为切片 A (无密度数据)"); action_b.setText("设为切片 B (无密度数据)")
        menu.addAction(action_a); menu.addAction(action_b)
    ...
    def _compute_logic_operation(self):
        ...
        idx_a = self.selected_slice_a; idx_b = self.selected_slice_b; operation_str = self.logic_op_combo.currentText()
        matrix_a = self.density_matrices.get(idx_a); matrix_b = self.density_matrices.get(idx_b)
        operation_func = self.LOGIC_OPERATIONS.get(operation_str)
        if matrix_a is None or matrix_b is None or operation_func is None: return
        if matrix_a.shape != matrix_b.shape: QMessageBox.critical(self, "错误", "密度矩阵形状不匹配。"); return
        try:
            ...
            if '差分' in operation_str:
                 ...
                 vmin_res, vmax_res = -abs_max, abs_max
            else: vmin_res, vmax_res = 0, 1 # Assume binary 0/1 for non-diff logical ops
        ...
        except Exception as e: print(f"ERROR: Logic op failed: {e}"); QMessageBox.critical(self, "错误", f"运算失败: {e}"); self.logic_op_result_matrix = None; self.logic_op_result_pixmap = None
    ...
    def _update_batch_preview_display(self, slider_index):
        ...
        if 0 <= slider_index < len(indices):
            ...
            self.batch_results_label.setText(f"结果: {slider_index + 1} / {num_results} ({key[1]} vs {key[2]})") # Show indices used
    ...
    def _export_bitmaps(self, indices_to_export): # Consider renaming to _export_data
        ...
        global_params = {"export_time": datetime.datetime.now().isoformat(),"original_point_cloud_source": self.source_filename,"num_slices_param": self.num_slices_spin.value(),"slice_thickness_param": self.thickness_spin.value(), "limit_thickness": self.limit_thickness_check.isChecked()}
        ...
        exported_count = 0; exported_pcd_count = 0; exported_density_count = 0; exported_bitmap_count = 0
        for i, index in enumerate(indices_to_export):
             ...
             density_matrix = self.density_matrices.get(index); density_pixmap = self.density_pixmaps.get(index); density_params_saved = self.density_params.get(index)
             ...
             if not metadata.get("is_empty", False) and slice_data_pv is not None:
                 ...
                 except Exception as r_err: render_error_msg = str(r_err)
             ...
             except Exception as meta_err: print(f"ERROR saving meta {index}: {meta_err}")
             # Save Bitmap
             if img_np is not None:
                 try:
             ...
             if density_matrix is not None:
                 try:
                     ...
                     if density_pixmap and not density_pixmap.isNull():
                         ...
                         density_saved = True; exported_density_count += 1
                 except Exception as den_err:
                     print(f"ERROR saving density {index}: {den_err}")
             elif index in self.density_matrices: density_saved = True # Attempted
